"""Create an interactive HTML graph from Neo4j data.

Uses pyvis to render nodes and edges into `neo4j_viz.html`.
"""
...
# sync driver used for visualization
driver = GraphDatabase.driver(config.NEO4J_URI, auth=(config.NEO4J_USER, config.NEO4J_PASSWORD))


def fetch_subgraph(tx, limit=500):
    """Fetch a limited set of (a)-[r]->(b) rows for visualization."""
    q = (
...
def build_pyvis(rows, output_html="neo4j_viz.html"):
    """Build and save the pyvis HTML file from DB rows."""
